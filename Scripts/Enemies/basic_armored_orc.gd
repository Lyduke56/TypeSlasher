...
# Block mechanism - one-time defense against first attack
...
func set_targeted_state(targeted: bool):
	...
	if targeted:
		...
		if anim:
			anim.play("armored_idle")
		modulate = Color.GRAY  # Darken the enemy
	else:
		modulate = Color.WHITE  # Reset color

func play_death_animation():
	"""Play damage/block animation followed by death animation"""
	if not anim:
		queue_free()  # Fallback if no animation
		return

	# Prevent multiple animations on the same enemy
	if anim.animation == "armored_death" or anim.animation == "armored_damaged" or anim.animation == "armored_block":
		return

	if not has_blocked:
		# First time - play block animation directly and mark as blocked
		has_blocked = true
		print("Enemy blocking first attack directly!")
		...
		# Connect signal and play block animation
		anim.animation_finished.connect(_on_block_animation_finished, CONNECT_ONE_SHOT)
		anim.play("armored_block")
		print("Enemy block animation started (direct)")
	else:
		# Already blocked - play damage animation leading to death
		# Disconnect any existing connections to prevent duplicates
		...
		anim.play("armored_damaged")

		print("Enemy damage animation started")

func _on_damage_animation_finished():
	"""Called when damage animation completes"""
	if anim.animation == "armored_damaged":
		if not has_blocked:
			# First damage - play block animation
			has_blocked = true
			print("Enemy blocking first attack!")
			...
			anim.animation_finished.connect(_on_block_animation_finished, CONNECT_ONE_SHOT)
			anim.play("armored_block")
			print("Enemy block animation started")
		else:
			# Already blocked once - proceed to death
			# Disconnect any existing connections to prevent duplicates
...
func _on_block_animation_finished():
	...
	if anim.animation == "armored_block":
		...
		# Notify game system that enemy survived (optional - for potential sound/effects)
		# You could emit a signal here if needed

func _on_death_animation_finished():
	"""Called when death animation completes"""
	if anim.animation == "armored_death":
...
func _on_hack_timer_timeout():
	...
	if has_reached_target and not is_being_targeted and anim:
		...
		anim.sprite_frames.set_animation_loop("armored_attack_2", false)
		anim.play("armored_attack_2")
		print("Enemy hacking the target!")

func _on_animation_finished():
	"""Called when any animation finishes"""
	# Don't interfere with death/damage animations or when enemy is being targeted
	if anim and (anim.animation == "armored_death" or anim.animation == "armored_damaged" or is_being_targeted):
		return

	if has_reached_target and anim and anim.animation == "armored_attack_2":
...
func _physics_process(delta: float) -> void:
	...
	if is_being_targeted or has_reached_target:
		...
		if anim and (anim.animation == "armored_death" or anim.animation == "armored_damaged"):
		...
		if anim and has_reached_target and not is_being_targeted and anim.animation != "armored_attack_2":
			anim.play("armored_idle")
