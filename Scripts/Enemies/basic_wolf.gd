...
func play_death_animation():
	...
	if anim.animation == "werewolf_death" or anim.animation == "werewolf_damaged":
		return
...
func _on_damage_animation_finished():
	...
	if anim.animation == "werewolf_damaged":
		# Disconnect any existing connections to prevent duplicates
...
func _on_hack_timer_timeout():
	...
	if has_reached_target and not is_being_targeted and anim:
		...
		anim.sprite_frames.set_animation_loop("werewolf_attack_2", false)
		anim.play("werewolf_attack_2")
		print("Enemy hacking the target!")

func _on_animation_finished():
	"""Called when any animation finishes"""
	# Don't interfere with death/damage animations or when enemy is being targeted
	if anim and (anim.animation == "werewolf_death" or anim.animation == "werewolf_damaged" or is_being_targeted):
		return

	if has_reached_target and anim and anim.animation == "werewolf_attack_2":
...
func _physics_process(delta: float) -> void:
	...
	if is_being_targeted or has_reached_target:
		...
		if anim and (anim.animation == "werewolf_death" or anim.animation == "werewolf_damaged"):
		...
		if anim and has_reached_target and not is_being_targeted and anim.animation != "werewolf_attack_2":
			anim.play("werewolf_idle")
