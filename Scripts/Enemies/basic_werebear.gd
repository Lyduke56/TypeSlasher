...
func play_death_animation():
	...
	if get("is_frozen"): # Use get() to be safe if variable is missing, or access directly if extends enemy_base
		set("is_frozen", false)
		var vines = get("freeze_vines")
		if vines:
			vines.visible = false

		# Restore original frames so specific animations (death) exist
		var orig_frames = get("original_sprite_frames")
		if orig_frames and anim.sprite_frames != orig_frames:
			anim.sprite_frames = orig_frames
	# -----------------------------------------------------------
	...
	if anim.animation == "werebear_death" or anim.animation == "werebear_damaged":
		return
...
func _on_damage_animation_finished():
	...
	if anim.animation == "werebear_damaged":
		# Disconnect any existing connections to prevent duplicates
...
func _on_death_animation_finished():
	...
	if anim.animation == "werebear_death":
...
func _on_hack_timer_timeout():
	"""Called every 1.5 seconds to play double attack animation"""
...
func _on_animation_finished():
	...
	if anim and (anim.animation == "werebear_death" or anim.animation == "werebear_damaged" or is_being_targeted):
		return

	if has_reached_target and anim and (anim.animation == "werebear_double_attack" or anim.animation == "werebear_attack_2"):
		# Double attack animation finished, go back to idle and deal 2 damages
		anim.play("werebear_idle")
		print("Enemy finished double attack, back to idle.")
		if target_node:
			target_node.take_damage()  # First damage
...
func _physics_process(delta: float) -> void:
	...
	if is_being_targeted or has_reached_target:
		...
		if anim and (anim.animation == "werebear_death" or anim.animation == "werebear_damaged"):
		...
		if anim and has_reached_target and not is_being_targeted and anim.animation != "werebear_double_attack" and anim.animation != "werebear_attack_2":
			anim.play("werebear_idle")
