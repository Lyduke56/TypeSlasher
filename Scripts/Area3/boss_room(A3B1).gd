...
func _spawn_enemy_at_position(spawn_position: Vector2, enemy_scene: PackedScene):
	...
	if is_boss_enemy:
		if is_demon_boss:
			...
			if spawn_locations:
				...
				if top_node:
					final_spawn_position = top_node.global_position
					print("Demon boss detected - spawning at Top node: " + str(top_node.global_position))
				else:
					print("ERROR: Demon boss detected but Top node not found in BossRoomSpawn/SpawnLocations - using assigned position")
			else:
				print("ERROR: BossRoomSpawn/SpawnLocations not found - using assigned position")
		else:
			# Other bosses use BossSpawnPoint
			var spawn_locations = get_node_or_null("BossRoomSpawn/SpawnLocations")
	...
	print("Final position:", final_spawn_position)
...
func _get_unique_word_for_category(category: String) -> String:
	...
	if unused_words.size() > 0:
		...
		if available_words.size() > 0:
			var selected_word = available_words[randi() % available_words.size()]
			print("Fireball spawned with tracked word: '", selected_word, "' (easy difficulty)")
...
func _on_enemy_died():
	...
	if enemies_remaining <= 0 and not is_spawning_enemies:
		...
		# For boss rooms, clear immediately when all enemies defeated
...
func _spawn_demon_boss():
	...
	boss_instance.position = top_node.global_position
	...
	print("Summoning DEMON BOSS at Top node:", top_node.global_position)
	print("Moving towards target at:", target_position)
	...
	print("DEBUG: Area3 room set associated_room to self using set() method")
	...
	boss_spawned = true
	is_spawning_enemies = false  # Boss room doesn't use wave spawning

	print("DEMON BOSS SPAWNED - Unique targeting mechanics activated!")
