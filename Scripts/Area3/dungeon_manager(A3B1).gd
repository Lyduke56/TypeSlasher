...
func setup_room_connections():
	...
	starting.set_connected_room("bottom", room_c)

	room_a.set_connected_room("bottom", healing)
	room_a.set_connected_room("left", starting)

	room_b.set_connected_room("top", boss)
	room_b.set_connected_room("bottom", starting)

	room_c.set_connected_room("top", starting)

	healing.set_connected_room("top", room_a)
...
func _input(event):
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if direction != "" and current_room != null and current_room.exit_markers.has(direction) and not self.is_transitioning:
			# Prevent room transitions if player is currently targeting an enemy
			if active_enemy != null:
				print("Cannot transition - player is targeting an enemy")
				return
