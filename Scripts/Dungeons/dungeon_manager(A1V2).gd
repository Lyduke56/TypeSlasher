...
func setup_room_connections():
	...
	starting.set_connected_room("top", room_e)
	...
	room_c.set_connected_room("bottom", portal)

	room_d.set_connected_room("top", starting)

	room_e.set_connected_room("bottom", starting)

	portal.set_connected_room("top", room_c)
...
func transition_to_room(direction: String):
	...
	tween.tween_callback(func():
		...
		prompt_cleared.emit()  # After active_enemy = null
