...
func setup_room_connections():
	...
	var room_a = get_node("../RoomA - Small")
	var room_b = get_node("../RoomB - Medium")
	var room_c = get_node("../RoomC - Medium")
	var room_d = get_node("../RoomD - Hard")
	var portal = get_node("../PortalRoom")

	starting.set_connected_room("left", room_a)

	room_a.set_connected_room("right", starting)
	room_a.set_connected_room("bottom", room_b)

	room_b.set_connected_room("top", room_a)
	room_b.set_connected_room("right", room_c)

	room_c.set_connected_room("left", room_b)
	room_c.set_connected_room("right", portal)
	room_c.set_connected_room("bottom", room_d)

	room_d.set_connected_room("top", room_c)

	portal.set_connected_room("left", room_c)
	...
	if current_room and current_room.has_node("EnemyContainer"):
		...
		for entity in enemy_container.get_children():
			...
			if prompt.length() > 0 and prompt.substr(0, 1).to_lower() == typed_character:
				...
				prompt_updated.emit(prompt, current_letter_index)  # After the existing code
				break
