...
var sword_buff_stacks: int = 0
var sword_heal_chance: int = 0

var freeze_buff_stacks: int = 0
var freeze_activation_chance: int = 0
...
func _find_enemy_nodes(node: Node, enemies: Array):
	if node.has_method("pause_enemy") and node.get("is_dying") != true and node.get("is_frozen") != true and node.get("is_being_targeted") != true:
		enemies.append(node)
